...
function RentalCalc({saved,onCalcChange,profile,isPro:isProProp,onActivatePro,allDeals=[],currentDealId=null}) {
  ...
  const strategyMode = "hybrid";
  const riskTol = strategyMode;
    ...
    <div style={{display:"flex",flexDirection:"column",gap:0,width:"100%",boxSizing:"border-box"}}>
      ...
      <div style={{background:"linear-gradient(160deg,#0a0f1e 0%,#0f172a 60%,#1a2340 100%)",borderRadius:20,padding:"22px 20px 18px",marginBottom:16,boxShadow:"0 8px 32px rgba(0,0,0,0.28)",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:-40,right:-40,width:160,height:160,borderRadius:"50%",background:`radial-gradient(circle,${vColor}18 0%,transparent 70%)`,pointerEvents:"none"}}/>
                ...
                {composite.highRiskCap&&(
          <div style={{marginBottom:10}}>
            <span style={{fontSize:9,fontWeight:700,color:"#fbbf24",background:"#78350f40",padding:"3px 10px",borderRadius:100,letterSpacing:"0.05em"}}>⚠ RISK CAP ACTIVE</span>
          </div>
        )}
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
        ...
        {/* Property & Financing */}
        <InputSection title="Property & Financing" accent="#10b981" defaultOpen badge={`${fmtD(c.effPP)} · ${fmtD(mort)}/mo`}>
          ...
          {/* Income inline */}
          <div style={{borderTop:"1px solid #f0f0f0",marginTop:10,paddingTop:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          {/* Growth assumptions inline */}
          <div style={{borderTop:"1px solid #f0f0f0",marginTop:8,paddingTop:8}}>
            <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:6}}>Long-Term Assumptions</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fafafa"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Stability Check</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>DSCR · Vacancy stress · Break-even occupancy</div>
          </div>
          {!isPro&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"#fffbeb",border:"1px solid #fde68a",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Partial</span>}
        </div>
        <div style={{padding:"14px 18px"}}>
          {/* DSCR bar — always free */}
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"10px 12px",background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em"}}>DSCR</span>
                <span style={{fontSize:13,fontWeight:900,fontFamily:"'DM Mono',monospace",color:dscr>=1.25?"#059669":dscr>=1.0?"#d97706":"#dc2626"}}>{dscr.toFixed(2)}x</span>
              </div>
              <div style={{height:6,background:"#e5e7eb",borderRadius:100,overflow:"hidden"}}>
                <div style={{height:"100%",width:Math.min(100,(dscr/2)*100)+"%",background:dscr>=1.25?"#059669":dscr>=1.0?"#d97706":"#dc2626",borderRadius:100,transition:"width 0.4s"}}/>
              </div>
              <div style={{fontSize:9,color:"#9ca3af",marginTop:3}}>Safe threshold: 1.25x · Minimum: 1.0x</div>
            </div>
          </div>
          {/* 1 free stress test */}
          {risk.scenarios&&risk.scenarios.slice(0,1).map((s,si)=>(
            <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:s.breaches?"#fef2f2":"#f0fdf4",borderRadius:10,border:`1px solid ${s.breaches?"#fecaca":"#6ee7b7"}`,marginBottom:12}}>
              <span style={{fontSize:18}}>{s.breaches?"⚠️":"✅"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:700,color:"#111827"}}>Stress test: {s.label}</div>
                <div style={{fontSize:10,color:"#6b7280",marginTop:1}}>Cash flow: <span style={{fontWeight:700,color:s.cf>=0?"#059669":"#dc2626",fontFamily:"'DM Mono',monospace"}}>{s.cf>=0?"+":""}{Math.round(s.cf).toLocaleString()}/mo</span> · DSCR: <span style={{fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{s.dscr.toFixed(2)}x</span></div>
              </div>
              {s.breaches&&<span style={{fontSize:8,fontWeight:700,color:"#dc2626",background:"#fee2e2",padding:"2px 6px",borderRadius:100}}>BREACH</span>}
            </div>
          ))}
          {/* FREE lock — rest of suite */}
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#f0fdf4)",borderRadius:10,border:"1.5px dashed #d1fae5",padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>
                {risk.scenarios&&risk.scenarios[0]&&!risk.scenarios[0].breaches
                  ? "✅ Passed 1 of 6 stress tests. Pro runs all 6."
                  : "⚠️ Failed the first stress test. Pro shows all 6 — and how to fix them."}
              </div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10}}>Unlock: 2-mo/3-mo vacancy · Rate +0.5%/+1%/+2% · Expense blowout · Fragility score · Break-even occupancy</div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#064e3b,#065f46)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Full Stability Suite →</button>
            </div>
          )}
          {/* PRO: full stress suite */}
          {isPro&&(
            <>
              <div style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>All Stress Scenarios</div>
              <div style={{display:"flex",gap:6,marginBottom:10}}>
                {["vacancy","interest","expense"].map(t=>(
                  <button key={t} onClick={()=>setStressTab(t)} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab===t?"#111827":"#f3f4f6",color:stressTab===t?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>{t}</button>
                ))}
              </div>
              <div style={{borderRadius:10,border:"1px solid #f3f4f6",overflow:"hidden",marginBottom:12}}>
                {risk.scenarios.filter(s=>s.cat===stressTab).map((s,si,arr)=>(
                  <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderBottom:si<arr.length-1?"1px solid #f9fafb":"none",background:s.breaches?"#fef9f9":"white"}}>
                    ...
                    <span style={{fontSize:11,fontFamily:"'DM Mono',monospace",color:s.cf>=0?"#059669":"#dc2626",width:65,textAlign:"right"}}>{s.cf>=0?"+":""}{Math.round(s.cf).toLocaleString()}/mo</span>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                <div style={{background:"#f8fafc",borderRadius:8,padding:"10px 12px"}}>
                  <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",textTransform:"uppercase",marginBottom:4}}>Break-Even Occupancy</div>
                  <div style={{fontSize:16,fontWeight:900,fontFamily:"'DM Mono',monospace",color:beo<=0.85?"#059669":beo<=0.92?"#d97706":"#dc2626"}}>{Math.round(beo*100)}%</div>
                  <div style={{fontSize:9,color:"#9ca3af",marginTop:2}}>Safe: under 85%</div>
                </div>
                <div style={{background:"#f8fafc",borderRadius:8,padding:"10px 12px"}}>
                  <div style={{fontSize:9,fontWeight:700,color:"#9ca3af",textTransform:"uppercase",marginBottom:4}}>Fragility Score</div>
                  <div style={{fontSize:16,fontWeight:900,fontFamily:"'DM Mono',monospace",color:fragility<30?"#059669":fragility<60?"#d97706":"#dc2626"}}>{fragility}/100</div>
                  <div style={{fontSize:9,color:"#9ca3af",marginTop:2}}>Lower = more resilient</div>
                </div>
              </div>
            </>
          )}
...
{/* ══════════════════════════════════════════════════════════════════════
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        ...
        <div style={{padding:"16px 18px"}}>
          {/* CoC + Capital In — always free */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
            {[
              {l:"Cash-on-Cash ROI", v:fmtP(c.coc), col:c.coc>=0.08?"#059669":c.coc>=0.04?"#d97706":"#dc2626"},
          ...
          {/* Pro gate for deep capital analysis */}
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#eff6ff)",borderRadius:10,border:"1.5px dashed #bfdbfe",padding:"14px 16px"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>🔒 Pro unlocks full capital engineering</div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10,lineHeight:1.5}}>
                5-yr equity · Capital multiplier (5/10/20yr) · Years to capital recovery · Optimal refi windows (yr 3/5/7/10) · IRR projection
              </div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#1e3a8a,#1d4ed8)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Capital Engineering →</button>
            </div>
          )}
          {isPro&&(<>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Pro Analysis Suite</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Capital Efficiency · Optimization Engine · 30-Year Wealth Projection</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>What Pro unlocks for this deal:</div>
          <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Cash-on-cash return, IRR, equity multiple, and annualized ROI</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Optimization sliders — lower down payment, early refi, or rent growth impact</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Monte Carlo simulation across 1,000 scenarios with probability of profit</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>30-year wealth curve: appreciation, equity paydown, and net worth trajectory</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Year-by-year cashflow table and 3 exit scenario comparisons</span></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#064e3b,#065f46)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:"white",marginBottom:2}}>Upgrade to Pro</div>
              <div style={{fontSize:11,color:"rgba(255,255,255,0.65)"}}>$20/mo · Cancel anytime · Instant access</div>
            </div>
            <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"10px 22px",borderRadius:8,border:"1.5px solid rgba(255,255,255,0.3)",background:"rgba(255,255,255,0.12)",color:"white",fontSize:13,fontWeight:700,cursor:"pointer"}}>Upgrade Now →</button>
          </div>
        </div>
      </div>
      )}
...
{/* ══════════════════════════════════════════════════════════════════════
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>30-Year Wealth Projection</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Wealth curve · Year-by-year table · Exit scenario comparison</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>See the full 30-year picture for this rental:</div>
          <div style={{marginBottom:16}}>
            {["30-year net worth curve with appreciation and equity paydown","Year-by-year cash flow, equity, and ROI table","3 exit scenario comparisons: sell, refi, or hold forever","Portfolio impact — how this deal affects your total monthly cash flow"].map(b=>(
              <div key={b} style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}>
                <span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span>
    ...
    {/* SECTION 2 — STABILITY (spread protection / stress) */}
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fafafa"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>STABILITY CHECK</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Spread protection · Repair risk · Fee safety</div>
          </div>
          {!isPro&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"#fffbeb",border:"1px solid #fde68a",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Partial</span>}
        </div>
        <div style={{padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"10px 12px",background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em"}}>STABILITY CHECK</span>
                <span style={{fontSize:13,fontWeight:900,fontFamily:"'DM Mono',monospace",color:wsRisk.spreadScore/50>=1.25?"#059669":wsRisk.spreadScore/50>=1.0?"#d97706":"#dc2626"}}>{(wsRisk.spreadScore/50).toFixed(2)}x</span>
              </div>
              <div style={{height:6,background:"#e5e7eb",borderRadius:100,overflow:"hidden"}}><div style={{height:"100%",width:Math.min(100,(wsRisk.spreadScore/50/2)*100)+"%",background:wsRisk.spreadScore/50>=1.25?"#059669":wsRisk.spreadScore/50>=1.0?"#d97706":"#dc2626",borderRadius:100,transition:"width 0.4s"}}/></div>
              <div style={{fontSize:9,color:"#9ca3af",marginTop:3}}>Safe: 1.25x · Minimum: 1.0x</div>
            </div>
          </div>
          {stressSims&&stressSims.slice(0,1).map((s,si)=>(
            <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:s.breaches?"#fef2f2":"#f0fdf4",borderRadius:10,border:`1px solid ${s.breaches?"#fecaca":"#6ee7b7"}`,marginBottom:12}}>
              <span style={{fontSize:18}}>{s.breaches?"⚠️":"✅"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:700,color:"#111827"}}>Stress: {s.label}</div>
                <div style={{fontSize:10,color:"#6b7280",marginTop:1}}>Cash flow: <span style={{fontWeight:700,color:s.cf>=0?"#059669":"#dc2626",fontFamily:"'DM Mono',monospace"}}>{s.cf>=0?"+":""}{Math.round(s.cf).toLocaleString()}/mo</span></div>
              </div>
              {s.breaches&&<span style={{fontSize:8,fontWeight:700,color:"#dc2626",background:"#fee2e2",padding:"2px 6px",borderRadius:100}}>BREACH</span>}
            </div>
          ))}
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#f0fdf4)",borderRadius:10,border:"1.5px dashed #d1fae5",padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>{ stressSims&&stressSims[0]&&!stressSims[0].breaches ? "✅ Spread survives scenario 1. Pro stress-tests 4 more." : "⚠️ Spread is fragile. Pro shows all 4 scenarios and how to fix your offer." }</div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10}}>Unlock: ARV -5%/-10% · Repair +15% · Fee compression · Margin fragility score</div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#064e3b,#065f46)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Full Stability Suite →</button>
            </div>
          )}
          {isPro&&(
            <>
              <div style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>All Stress Scenarios</div>
              <div style={{display:"flex",gap:6,marginBottom:10}}>  <button key="spread" onClick={()=>setStressTab("spread")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="spread"?"#111827":"#f3f4f6",color:stressTab==="spread"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>spread</button>  </div>
              <div style={{borderRadius:10,border:"1px solid #f3f4f6",overflow:"hidden",marginBottom:8}}>{ stressSims.filter(s=>s.cat===stressTab).map((s,si,arr)=>(
                <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderBottom:si<arr.length-1?"1px solid #f9fafb":"none",background:s.breaches?"#fef9f9":"white"}}>
                  <span style={{fontSize:10,color:"#6b7280",width:90,flexShrink:0}}>{s.label}</span>
                  <div style={{flex:1,height:4,background:"#f3f4f6",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(0,Math.min(100,(s.dscr/2)*100))+"%",background:s.breaches?"#dc2626":"#059669",borderRadius:2}}/></div>
                  <span style={{fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace",color:s.breaches?"#dc2626":"#374151",width:36,textAlign:"right"}}>{s.dscr.toFixed(2)}x</span>
                  <span style={{fontSize:11,fontFamily:"'DM Mono',monospace",color:s.cf>=0?"#059669":"#dc2626",width:65,textAlign:"right"}}>{s.cf>=0?"+":""}{Math.round(s.cf).toLocaleString()}/mo</span>
                  {s.breaches&&<span style={{fontSize:8,color:"#dc2626",background:"#fef2f2",padding:"1px 4px",borderRadius:100,fontWeight:700,flexShrink:0}}>BREACH</span>}
                </div>
    ...
    <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Pro Analysis Suite</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Capital Efficiency · Fee Optimization · Deal Pipeline Projection</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>What Pro unlocks for this deal:</div>
          <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Assignment fee ROI and capital velocity score per deal</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Fee optimization — maximum fee at your buyer's target margin</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Monthly deal flow projections and annualized wholesale income modeling</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Comparable deal benchmarking against market spreads</span></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#064e3b,#065f46)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:"white",marginBottom:2}}>Upgrade to Pro</div>
    ...
    {/* SECTION 2 — STABILITY */}
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fafafa"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>STABILITY CHECK</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Profit survival · ARV stress · Timeline risk</div>
          </div>
          {!isPro&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"#fffbeb",border:"1px solid #fde68a",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Partial</span>}
        </div>
        <div style={{padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"10px 12px",background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em"}}>STABILITY CHECK</span>
                <span style={{fontSize:13,fontWeight:900,fontFamily:"'DM Mono',monospace",color:(flipRisk&&flipRisk.profit>0?1.3:0.8)>=1.25?"#059669":(flipRisk&&flipRisk.profit>0?1.3:0.8)>=1.0?"#d97706":"#dc2626"}}>{((flipRisk&&flipRisk.profit>0?1.3:0.8)).toFixed(2)}x</span>
              </div>
              <div style={{height:6,background:"#e5e7eb",borderRadius:100,overflow:"hidden"}}><div style={{height:"100%",width:Math.min(100,((flipRisk&&flipRisk.profit>0?1.3:0.8)/2)*100)+"%",background:(flipRisk&&flipRisk.profit>0?1.3:0.8)>=1.25?"#059669":(flipRisk&&flipRisk.profit>0?1.3:0.8)>=1.0?"#d97706":"#dc2626",borderRadius:100,transition:"width 0.4s"}}/></div>
              <div style={{fontSize:9,color:"#9ca3af",marginTop:3}}>Safe: 1.25x · Minimum: 1.0x</div>
            </div>
          </div>
          {flipRisk&&flipRisk.stressSims||[]&&flipRisk&&flipRisk.stressSims||[].slice(0,1).map((s,si)=>(
            <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:s.breaches?"#fef2f2":"#f0fdf4",borderRadius:10,border:`1px solid ${s.breaches?"#fecaca":"#6ee7b7"}`,marginBottom:12}}>
              <span style={{fontSize:18}}>{s.breaches?"⚠️":"✅"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:700,color:"#111827"}}>Stress: {s.label}</div>
          ...
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#f0fdf4)",borderRadius:10,border:"1.5px dashed #d1fae5",padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>{ flipRisk&&flipRisk.stressSims||[]&&flipRisk&&flipRisk.stressSims||[][0]&&!flipRisk&&flipRisk.stressSims||[][0].breaches ? "✅ Profit survives the first stress scenario. Pro models 4 more." : "⚠️ Profit is at risk. Pro shows all 4 scenarios and your break-even ARV." }</div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10}}>Unlock: ARV -5%/-10% · Hold +30d/60d · Repairs +15% · Break-even ARV · Annualized ROI</div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#064e3b,#065f46)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Full Stability Suite →</button>
            </div>
          )}
          {isPro&&(
            <>
              <div style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>All Stress Scenarios</div>
              <div style={{display:"flex",gap:6,marginBottom:10}}>  <button key="arv" onClick={()=>setStressTab("arv")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="arv"?"#111827":"#f3f4f6",color:stressTab==="arv"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>arv</button><button key="timeline" onClick={()=>setStressTab("timeline")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="timeline"?"#111827":"#f3f4f6",color:stressTab==="timeline"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>timeline</button>  </div>
              <div style={{borderRadius:10,border:"1px solid #f3f4f6",overflow:"hidden",marginBottom:8}}>{ flipRisk&&flipRisk.stressSims||[].filter(s=>s.cat===stressTab).map((s,si,arr)=>(
                <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderBottom:si<arr.length-1?"1px solid #f9fafb":"none",background:s.breaches?"#fef9f9":"white"}}>
                  <span style={{fontSize:10,color:"#6b7280",width:90,flexShrink:0}}>{s.label}</span>
                  <div style={{flex:1,height:4,background:"#f3f4f6",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(0,Math.min(100,(s.dscr/2)*100))+"%",background:s.breaches?"#dc2626":"#059669",borderRadius:2}}/></div>
                  <span style={{fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace",color:s.breaches?"#dc2626":"#374151",width:36,textAlign:"right"}}>{s.dscr.toFixed(2)}x</span>
    ...
    <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Pro Analysis Suite</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Capital Efficiency · Profit Optimization · Exit Scenario Modeling</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>What Pro unlocks for this deal:</div>
          <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Annualized ROI, IRR, and equity multiple on total invested capital</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Profit optimization — ideal sale price, carry reduction, and rehab value-add ROI</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Exit scenario comparison: sell now vs rent vs seller-finance</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>30-day, 60-day, and 90-day profit projections by exit timeline</span></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#064e3b,#065f46)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:"white",marginBottom:2}}>Upgrade to Pro</div>
  ...
  return (<>
    ...
    {/* SECTION 2 — STABILITY */}
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fafafa"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>REFI DSCR</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Refi DSCR · Capital recovery · ARV sensitivity</div>
          </div>
          {!isPro&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"#fffbeb",border:"1px solid #fde68a",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Partial</span>}
        </div>
        <div style={{padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"10px 12px",background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em"}}>REFI DSCR</span>
                <span style={{fontSize:13,fontWeight:900,fontFamily:"'DM Mono',monospace",color:(brrrRisk&&brrrRisk.dscr)||1>=1.25?"#059669":(brrrRisk&&brrrRisk.dscr)||1>=1.0?"#d97706":"#dc2626"}}>{((brrrRisk&&brrrRisk.dscr)||1).toFixed(2)}x</span>
              </div>
              <div style={{height:6,background:"#e5e7eb",borderRadius:100,overflow:"hidden"}}><div style={{height:"100%",width:Math.min(100,((brrrRisk&&brrrRisk.dscr)||1/2)*100)+"%",background:(brrrRisk&&brrrRisk.dscr)||1>=1.25?"#059669":(brrrRisk&&brrrRisk.dscr)||1>=1.0?"#d97706":"#dc2626",borderRadius:100,transition:"width 0.4s"}}/></div>
              <div style={{fontSize:9,color:"#9ca3af",marginTop:3}}>Safe: 1.25x · Minimum: 1.0x</div>
            </div>
          </div>
          {brrrRisk&&brrrRisk.stressSims||[]&&brrrRisk&&brrrRisk.stressSims||[].slice(0,1).map((s,si)=>(
            <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:s.breaches?"#fef2f2":"#f0fdf4",borderRadius:10,border:`1px solid ${s.breaches?"#fecaca":"#6ee7b7"}`,marginBottom:12}}>
              <span style={{fontSize:18}}>{s.breaches?"⚠️":"✅"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:700,color:"#111827"}}>Stress: {s.label}</div>
          ...
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#f0fdf4)",borderRadius:10,border:"1.5px dashed #d1fae5",padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>{ brrrRisk&&brrrRisk.stressSims||[]&&brrrRisk&&brrrRisk.stressSims||[][0]&&!brrrRisk&&brrrRisk.stressSims||[][0].breaches ? "✅ Refi holds in the base scenario. Pro models 5 ARV and rate scenarios." : "⚠️ Refi DSCR is tight. Pro shows how much ARV must hold — and your cash-left-in risk." }</div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10}}>Unlock: ARV ±5%/±10% · Rate +0.5%/+1% · Vacancy stress · Capital recovery % · Optimal timing</div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#064e3b,#065f46)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Full Stability Suite →</button>
            </div>
          )}
          {isPro&&(
            <>
              <div style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>All Stress Scenarios</div>
              <div style={{display:"flex",gap:6,marginBottom:10}}>  <button key="arv" onClick={()=>setStressTab("arv")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="arv"?"#111827":"#f3f4f6",color:stressTab==="arv"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>arv</button><button key="rate" onClick={()=>setStressTab("rate")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="rate"?"#111827":"#f3f4f6",color:stressTab==="rate"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>rate</button>  </div>
              <div style={{borderRadius:10,border:"1px solid #f3f4f6",overflow:"hidden",marginBottom:8}}>{ brrrRisk&&brrrRisk.stressSims||[].filter(s=>s.cat===stressTab).map((s,si,arr)=>(
                <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderBottom:si<arr.length-1?"1px solid #f9fafb":"none",background:s.breaches?"#fef9f9":"white"}}>
                  <span style={{fontSize:10,color:"#6b7280",width:90,flexShrink:0}}>{s.label}</span>
                  <div style={{flex:1,height:4,background:"#f3f4f6",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(0,Math.min(100,(s.dscr/2)*100))+"%",background:s.breaches?"#dc2626":"#059669",borderRadius:2}}/></div>
                  <span style={{fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace",color:s.breaches?"#dc2626":"#374151",width:36,textAlign:"right"}}>{s.dscr.toFixed(2)}x</span>
    ...
    <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Pro Analysis Suite</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Capital Efficiency · Refi Optimization · 30-Year Wealth Curve</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>What Pro unlocks for this deal:</div>
          <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>True capital recovery %, infinite return potential calculation</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Optimal refi timing — when to pull equity vs hold for appreciation</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Portfolio snowball projection — how this deal funds your next 3 acquisitions</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>30-year net worth trajectory including appreciation and loan paydown</span></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#064e3b,#065f46)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:"white",marginBottom:2}}>Upgrade to Pro</div>
  ...
  return (<>
    ...
    {/* SECTION 2 — STABILITY (DSCR / DOS / thin cash flow fragility) */}
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fafafa"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>DSCR</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>DSCR · Due-on-sale risk · Cash flow durability</div>
          </div>
          {!isPro&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"#fffbeb",border:"1px solid #fde68a",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Partial</span>}
        </div>
        <div style={{padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"10px 12px",background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em"}}>DSCR</span>
                <span style={{fontSize:13,fontWeight:900,fontFamily:"'DM Mono',monospace",color:(subRisk&&subRisk.dscr)||1>=1.25?"#059669":(subRisk&&subRisk.dscr)||1>=1.0?"#d97706":"#dc2626"}}>{((subRisk&&subRisk.dscr)||1).toFixed(2)}x</span>
              </div>
              <div style={{height:6,background:"#e5e7eb",borderRadius:100,overflow:"hidden"}}><div style={{height:"100%",width:Math.min(100,((subRisk&&subRisk.dscr)||1/2)*100)+"%",background:(subRisk&&subRisk.dscr)||1>=1.25?"#059669":(subRisk&&subRisk.dscr)||1>=1.0?"#d97706":"#dc2626",borderRadius:100,transition:"width 0.4s"}}/></div>
              <div style={{fontSize:9,color:"#9ca3af",marginTop:3}}>Safe: 1.25x · Minimum: 1.0x</div>
            </div>
          </div>
          {subRisk&&subRisk.stressSims||[]&&subRisk&&subRisk.stressSims||[].slice(0,1).map((s,si)=>(
            <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:s.breaches?"#fef2f2":"#f0fdf4",borderRadius:10,border:`1px solid ${s.breaches?"#fecaca":"#6ee7b7"}`,marginBottom:12}}>
              <span style={{fontSize:18}}>{s.breaches?"⚠️":"✅"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:700,color:"#111827"}}>Stress: {s.label}</div>
          ...
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#f0fdf4)",borderRadius:10,border:"1.5px dashed #d1fae5",padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>{ subRisk&&subRisk.stressSims||[]&&subRisk&&subRisk.stressSims||[][0]&&!subRisk&&subRisk.stressSims||[][0].breaches ? "✅ Cash flow holds in scenario 1. Pro runs due-on-sale risk + 4 more." : "⚠️ Cash flow is thin. Pro shows due-on-sale risk score and 4 more scenarios." }</div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10}}>Unlock: Due-on-sale risk score · Rate arbitrage durability · 1-mo/3-mo vacancy</div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#064e3b,#065f46)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Full Stability Suite →</button>
            </div>
          )}
          {isPro&&(
            <>
              <div style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>All Stress Scenarios</div>
              <div style={{display:"flex",gap:6,marginBottom:10}}>  <button key="vacancy" onClick={()=>setStressTab("vacancy")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="vacancy"?"#111827":"#f3f4f6",color:stressTab==="vacancy"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>vacancy</button><button key="dos" onClick={()=>setStressTab("dos")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="dos"?"#111827":"#f3f4f6",color:stressTab==="dos"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>dos</button>  </div>
              <div style={{borderRadius:10,border:"1px solid #f3f4f6",overflow:"hidden",marginBottom:8}}>{ subRisk&&subRisk.stressSims||[].filter(s=>s.cat===stressTab).map((s,si,arr)=>(
                <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderBottom:si<arr.length-1?"1px solid #f9fafb":"none",background:s.breaches?"#fef9f9":"white"}}>
                  <span style={{fontSize:10,color:"#6b7280",width:90,flexShrink:0}}>{s.label}</span>
                  <div style={{flex:1,height:4,background:"#f3f4f6",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(0,Math.min(100,(s.dscr/2)*100))+"%",background:s.breaches?"#dc2626":"#059669",borderRadius:2}}/></div>
                  <span style={{fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace",color:s.breaches?"#dc2626":"#374151",width:36,textAlign:"right"}}>{s.dscr.toFixed(2)}x</span>
    ...
    <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Pro Analysis Suite</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Rate Arbitrage Efficiency · Optimization · Long-Term Hold Projection</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>What Pro unlocks for this deal:</div>
          <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Rate arbitrage yield — what the below-market rate is worth in actual dollars</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Equity capture efficiency compared to a traditional purchase</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Optimal exit timing analysis: sell, refi, or hold</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>30-year cash flow and equity accumulation projection</span></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#064e3b,#065f46)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:"white",marginBottom:2}}>Upgrade to Pro</div>
    ...
    {/* SECTION 2 — STABILITY (retail friction + ARV + timeline exposure) */}
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"12px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fafafa"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>STABILITY CHECK</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Profit stress · DOM risk · ARV compression</div>
          </div>
          {!isPro&&<span style={{fontSize:9,fontWeight:700,color:"#f59e0b",background:"#fffbeb",border:"1px solid #fde68a",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Partial</span>}
        </div>
        <div style={{padding:"14px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"10px 12px",background:"#f8fafc",borderRadius:10,border:"1px solid #e5e7eb"}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                <span style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em"}}>STABILITY CHECK</span>
                <span style={{fontSize:13,fontWeight:900,fontFamily:"'DM Mono',monospace",color:(novRisk&&novRisk.profit>0?1.3:0.8)>=1.25?"#059669":(novRisk&&novRisk.profit>0?1.3:0.8)>=1.0?"#d97706":"#dc2626"}}>{((novRisk&&novRisk.profit>0?1.3:0.8)).toFixed(2)}x</span>
              </div>
              <div style={{height:6,background:"#e5e7eb",borderRadius:100,overflow:"hidden"}}><div style={{height:"100%",width:Math.min(100,((novRisk&&novRisk.profit>0?1.3:0.8)/2)*100)+"%",background:(novRisk&&novRisk.profit>0?1.3:0.8)>=1.25?"#059669":(novRisk&&novRisk.profit>0?1.3:0.8)>=1.0?"#d97706":"#dc2626",borderRadius:100,transition:"width 0.4s"}}/></div>
              <div style={{fontSize:9,color:"#9ca3af",marginTop:3}}>Safe: 1.25x · Minimum: 1.0x</div>
            </div>
          </div>
          {novRisk&&novRisk.stressSims||[]&&novRisk&&novRisk.stressSims||[].slice(0,1).map((s,si)=>(
            <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",background:s.breaches?"#fef2f2":"#f0fdf4",borderRadius:10,border:`1px solid ${s.breaches?"#fecaca":"#6ee7b7"}`,marginBottom:12}}>
              <span style={{fontSize:18}}>{s.breaches?"⚠️":"✅"}</span>
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:700,color:"#111827"}}>Stress: {s.label}</div>
          ...
          {!isPro&&(
            <div style={{background:"linear-gradient(135deg,#f8fafc,#f0fdf4)",borderRadius:10,border:"1.5px dashed #d1fae5",padding:"14px 16px",textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:700,color:"#111827",marginBottom:6}}>{ novRisk&&novRisk.stressSims||[]&&novRisk&&novRisk.stressSims||[][0]&&!novRisk&&novRisk.stressSims||[][0].breaches ? "✅ Deal survives a 30-day listing. Pro models 60d and 90d+ scenarios." : "⚠️ Margin is thin at 30 days. Pro shows 60d/90d+ and your ARV floor." }</div>
              <div style={{fontSize:10,color:"#6b7280",marginBottom:10}}>Unlock: 60d/90d+ DOM · ARV -5%/-10% · Carrying cost burn rate · Agent friction score</div>
              <button onClick={()=>onActivatePro&&onActivatePro()} style={{padding:"8px 18px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#064e3b,#065f46)",color:"white",fontSize:11,fontWeight:700,cursor:"pointer"}}>Unlock Full Stability Suite →</button>
            </div>
          )}
          {isPro&&(
            <>
              <div style={{fontSize:10,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>All Stress Scenarios</div>
              <div style={{display:"flex",gap:6,marginBottom:10}}>  <button key="dom" onClick={()=>setStressTab("dom")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="dom"?"#111827":"#f3f4f6",color:stressTab==="dom"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>dom</button><button key="arv" onClick={()=>setStressTab("arv")} style={{padding:"4px 12px",borderRadius:100,border:"none",background:stressTab==="arv"?"#111827":"#f3f4f6",color:stressTab==="arv"?"white":"#6b7280",fontSize:10,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>arv</button>  </div>
              <div style={{borderRadius:10,border:"1px solid #f3f4f6",overflow:"hidden",marginBottom:8}}>{ novRisk&&novRisk.stressSims||[].filter(s=>s.cat===stressTab).map((s,si,arr)=>(
                <div key={si} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 12px",borderBottom:si<arr.length-1?"1px solid #f9fafb":"none",background:s.breaches?"#fef9f9":"white"}}>
                  <span style={{fontSize:10,color:"#6b7280",width:90,flexShrink:0}}>{s.label}</span>
                  <div style={{flex:1,height:4,background:"#f3f4f6",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:Math.max(0,Math.min(100,(s.dscr/2)*100))+"%",background:s.breaches?"#dc2626":"#059669",borderRadius:2}}/></div>
                  <span style={{fontSize:11,fontWeight:700,fontFamily:"'DM Mono',monospace",color:s.breaches?"#dc2626":"#374151",width:36,textAlign:"right"}}>{s.dscr.toFixed(2)}x</span>
    ...
    <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
      ...
      <div style={{background:"white",borderRadius:16,border:"1.5px solid #e5e7eb",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",marginBottom:12,overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:"1px solid #f3f4f6",display:"flex",alignItems:"center",justifyContent:"space-between",background:"linear-gradient(135deg,#f8fafc,#f0fdf4)"}}>
          <div>
            <div style={{fontSize:11,fontWeight:800,color:"#111827",textTransform:"uppercase",letterSpacing:"0.07em"}}>Pro Analysis Suite</div>
            <div style={{fontSize:10,color:"#9ca3af",marginTop:1}}>Capital Efficiency · Commission Optimization · Exit Scenario Comparison</div>
          </div>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",background:"#f0fdf4",border:"1px solid #6ee7b7",padding:"3px 10px",borderRadius:100,textTransform:"uppercase",letterSpacing:"0.07em"}}>🔒 Pro Only</span>
        </div>
        <div style={{padding:"20px 18px"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#111827",marginBottom:12}}>What Pro unlocks for this deal:</div>
          <div style={{marginBottom:16}}>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Net profit vs traditional listing commission comparison</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Commission structure optimization for maximum seller motivation</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Risk-adjusted return vs wholesale and fix-and-flip alternatives</span></div>
          <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8}}><span style={{fontSize:11,color:"#10b981",marginTop:1,flexShrink:0}}>✓</span><span style={{fontSize:12,color:"#374151",lineHeight:1.5}}>Deal velocity scoring — how fast this deal type closes in your market</span></div>
          </div>
          <div style={{background:"linear-gradient(135deg,#064e3b,#065f46)",borderRadius:12,padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div>
              <div style={{fontSize:13,fontWeight:800,color:"white",marginBottom:2}}>Upgrade to Pro</div>
